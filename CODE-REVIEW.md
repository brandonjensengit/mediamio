...
| Tests | F → **C** | 45 unit tests pass: 8 `PlaybackStreamURLBuilderTests` + 3 `ChapterTests` + 7 `SavedServersStoreTests` + 9 `ContentRatingTests` + 6 `QRCodeGeneratorTests` + 12 `PlaybackLifecycleControllerTests`. API client integration tests still needed. |
