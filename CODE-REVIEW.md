...
> **Status (2026-04-22):** Phases A + B + C + D all landed on `main` (Phase A: `f66c168`; Phase B: `8d467ac`, `b3f90f4`, `8f78f5d`; Phase C: `c3f1d6a`; Phase D first batch: `84d1544` — items #21, #24, #25, #26, #29; Phase D second batch: `b7b2e0b` — item #22 mDNS discovery; Phase D third batch: `16ad4d3` + `eacbcf9` + `d325926` — item #23 saved-servers / multi-user picker with silent re-login; Phase D fourth batch: `b3f59d4` — item #28 parental controls with PIN + content-rating filter; Phase D fifth batch: `b87da4d` — item #30 QR-code companion handoff). Phase D item #27 (offline downloads) is scoped out of this roadmap — revisit in a dedicated phase if prioritized. **Player lifecycle hardening** (post-Phase D; `bc81c00`) closes the two remaining video-player correctness items the review had flagged: `didBecomeActive` resume + `AVAudioSession.interruptionNotification` handling. 12 new unit tests lock the state machine. **Failover hardening + device-ID unification** (post-Phase D; `523db39`): `PlaybackFailoverController` no longer uses a 3-second hardcoded timer — it observes `playerItem.status`, `failedToPlayToEndTime` notifications, and a 15s watchdog gated on `isPlaybackBufferEmpty && !isPlaybackLikelyToKeepUp`, so slow-but-healthy connections no longer get demoted to transcode. New `DeviceIdentifier` utility consolidates the device ID source: `JellyfinAPIClient` and `VideoPlayerViewModel` had drifted onto two separate UserDefaults keys, breaking server-side session attribution. 17 new failover-decision tests + 4 device-identifier tests (62 total). **Phase B #11 closed** (post-Phase D; 2026-04-22): shared `Paginator<Item>` state machine (`Utilities/Paginator.swift`) replaces the duplicated pagination bookkeeping in `LibraryViewModel` and `SearchViewModel`; shared `EmptyStateView` (`Views/Components/EmptyStateView.swift`) replaces the five drifted empty-state views (`EmptyHomeView`, `EmptyLibraryView`, `SearchEmptyState`, `NoResultsView`, `LibrarySearchModal.emptySearchView`) plus the sixth `LibrarySearchModal.noResultsView` that the original audit missed. 15 new `PaginatorTests` lock the contract (first-page-replaces vs next-page-appends, total-record-count beats page-full heuristic, reload-resets-offset, reset() wipes, status-text formatting). 111 unit tests total.
...
| Video player | C → **A** | God-object decomposed; signal-based failover (no 3s timer false-positives); pause-state preserved across bitrate reload; lifecycle controller for app foreground/background + audio interruption; mid-playback bitrate reload; outro skip; `preferredMaximumResolution` capping HLS variants to display resolution; `MPNowPlayingInfoCenter` integration. DRM / PiP / AirPlay smoke-test still open. |
...
| Tests | F → **B-** | 111 unit tests pass: 8 `PlaybackStreamURLBuilderTests` + 3 `ChapterTests` + 7 `SavedServersStoreTests` + 9 `ContentRatingTests` + 6 `QRCodeGeneratorTests` + 12 `PlaybackLifecycleControllerTests` + 17 `PlaybackFailoverDecisionTests` + 4 `DeviceIdentifierTests` + 8 `AutoPlayPolicyTests` + 17 `MediaItemDecodingTests` + 5 `ItemsResponseDecodingTests` + 15 `PaginatorTests`. API client integration tests still needed. |
...
- ~~**`VideoPlayerViewModel.swift:1262–1263`** — `player.play()` called inside a Combine sink on status change. Hidden control flow; view cannot decide not to auto-play. Emit an event, let the view drive.~~ ✅ Closed by `f856ed0` (post-Phase D): the `.readyToPlay` sink now consults a small `AutoPlayPolicy` value-type instead of unconditionally calling `play()`. The reload path (`reloadWithCurrentSettings`) captures `isPlaying` before tearing down the player so a paused user who changes bitrate / audio quality stays paused after the new stream loads. Removed the duplicate `viewModel.startPlayback()` from `VideoPlayerView.onAppear` so the policy isn't bypassed when the SwiftUI representable re-mounts on reload. 8 `AutoPlayPolicyTests` cover the state machine (default, consume-and-reset, two consecutive paused reloads, recapture-overwrite).
- ~~**`VideoPlayerViewModel.swift:1374–1437`** — 3-second hardcoded delay before fallback kicks in. Network jitter causes false fallbacks (user gets transcoded stream when Direct Play would have succeeded). Observe `playable` or buffer-fill threshold instead.~~ ✅ Closed by `523db39` (post-Phase D): `PlaybackFailoverController` rewritten around AVPlayerItem signals — fast path observes `.status == .failed` + `failedToPlayToEndTime` notifications (immediate fallback for 404 / codec errors); slow-path 15s watchdog only fires when status is still `.unknown` AND buffer is empty AND `!isPlaybackLikelyToKeepUp`. Decision logic extracted to `PlaybackFailoverController.decide(_:)` pure function; 17 unit tests cover every branch including the bug-fix case (slow-but-healthy stream is left alone).
- ~~**`VideoPlayerViewModel.swift:1659`** — `reportPlaybackStart()` always sends `"PlayMethod": "DirectPlay"` regardless of actual mode.~~ ✅ Closed by Phase A decomposition (`f66c168`). The reporter (`Services/Playback/PlaybackSessionReporter.swift`) now takes a `PlaybackMode` parameter and maps it through `jellyfinPlayMethod(for:)` — DirectPlay / DirectStream / Remux→DirectStream / Transcode all attribute correctly. Audit entry was stale.
- ~~**`VideoPlayerView.swift:166–172`** — Pause on `willResignActive` is correct, but no resume on `didBecomeActive`. Background → foreground leaves playback paused. Also no handler for `AVAudioSession.interruptionNotification` (incoming call, Siri).~~ ✅ Closed by `PlaybackLifecycleController` (post-Phase D). App lifecycle + audio-interruption both flow through a single state machine: on `willResignActive` / interruption `.began`, record whether playback was active and pause; on `didBecomeActive`, resume if we were playing pre-background; on interruption `.ended` with `.shouldResume`, re-activate the shared `AVAudioSession` (required — otherwise `player.play()` silently no-ops) and resume. 12 unit tests cover every branch, including the activation-failure path.
- ~~**No `preferredMaximumResolution`** set on `AVURLAsset`. tvOS will auto-select the highest HLS variant even on 1080p displays, wasting bandwidth. Set based on `UIScreen.main.nativeBounds`.~~ ✅ Closed by Phase C item #20 (`c3f1d6a`). `VideoPlayerViewModel.startPlayback(url:mode:)` now sets `playerItem.preferredMaximumResolution` from `UIScreen.main.nativeBounds` so 1080p Apple TVs don't pull the 4K HLS variant.
- ~~**Bitrate picker UI is a lie.** `CustomInfoViewControllers.swift:132,259` posts bitrate-change notifications that the VM ignores.~~ ✅ Closed by Phase B (`b3f90f4`). VM observes `ReloadVideoWithNewBitrate` / `ReloadVideoWithNewAudioQuality` notifications and calls `reloadWithCurrentSettings()`, which tears down the active player and rebuilds via `PlaybackStreamURLBuilder` while preserving position via `pendingSeekOnReload`.
- ~~**No `MPNowPlayingInfoCenter` integration** — AirPlay / lock-screen / Siri remote don't show artwork or metadata.~~ ✅ Closed by Phase B (`b3f90f4`). New `NowPlayingPublisher` service publishes title / series / genre / year / artwork + playback position; wires `MPRemoteCommandCenter` play / pause / skip±10 / seek. Cleared on cleanup.
...
11. ✅ **Shared `Pagination` and `EmptyState` components** — review finding #4 closed. New `Paginator<Item>` (`Utilities/Paginator.swift`) is a `@MainActor final class` that owns `currentStartIndex` / `pageSize` / `hasMore` / `totalRecordCount` + the two mutual-exclusion flags (`isLoading` for page-zero reloads, `isLoadingMore` for tail-append). `LibraryViewModel` and `SearchViewModel` now compose it — each VM keeps its own `@Published` surface so SwiftUI wiring is unchanged, but the state-machine code is gone from both. A `PaginatedPage` protocol (with `ItemsResponse` as its only production conformer) lets tests mock pages without pulling in the full `MediaItem` graph. On the view side, new `EmptyStateView` replaces **six** bespoke empty states: `EmptyHomeView`, `EmptyLibraryView` (used by both `LibraryView` and `LibraryTabView`), `SearchEmptyState`, `NoResultsView`, `LibrarySearchModal.emptySearchView`, and `LibrarySearchModal.noResultsView` (the audit had only called out five — the sixth had the same layout but a `film.stack` icon for the "no results" branch). Component is parameterized on `systemImage` + `title` + optional `message` + optional `iconOpacity` (0.3 for modal contexts, 0.5 default) + optional primary `Action`. 15 new `PaginatorTests` lock the state machine against drift: first-page-replaces vs next-page-appends, total-record-count is authoritative (prevents wasted fetch when a 3/3 page is actually complete), page-full heuristic when no total, reload resets offset back to 0, `reset()` wipes everything, empty-page ends pagination cleanly, and all three status-text shapes.
12. ✅ **Unit tests on model decoding** — added by `dc319b6` (post-Phase D). 17 `MediaItemDecodingTests` + 5 `ItemsResponseDecodingTests` decode realistic Jellyfin JSON (movie / episode / series / minimal-payload / future-fields-tolerance / paged-list / empty-page / mid-pagination) and assert every field lands on the right Swift property. Round-trip tests catch asymmetric `CodingKeys`. Fixtures live in `MediaMioTests/Fixtures/JellyfinFixtures.swift` so the next test file can reuse the same payloads.
